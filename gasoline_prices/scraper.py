...
class Scraper:
    ...
    def get_newest_filename(self, url: str, current_datetime: datetime, user_agent: str) -> bool:
        ...
        with requests_get(self.base_url + url, stream=True, headers=headers) as res:
            ...
            if last_modified > current_datetime:
                excel_href: str = self.__get_excel_href(res.content)
                self.excel_href = excel_href
                return True
            else:
                return False

    def get_excel_url(self) -> str:
        excel_url: str = urljoin(self.base_url, self.excel_href)
        print(f"Excel URL: {excel_url}")
        return excel_url

    def __get_excel_href(self, raw_html: bytes) -> str:
        ...
        excel_href: str = result_excel_anchor["href"]
        return excel_href
